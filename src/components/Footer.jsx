    ...
    <footer className="text-white bg-primary">
              ...
              {email && (
                <div className="flex items-center space-x-4">
                  <Image
                    ...
                    priority
                  />
                  <a href={`mailto:${email}`} className="text-lg font-bold">{email}</a>
                </div>
              )}
            </div>
      ...
      <div className="bg-primary">
        <div className="container pb-24">
          {menu.length > 0 && (
            <ul className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-xl font-extrabold uppercase tracking-wide">
              {menu.map((item, idx) => {
                ...
                return (
                  <li key={idx} className="text-white hover:secondary-color">
                    {isExt ? (
                      <a href={item.link || '#'} target={target} rel={rel}>{item.text}</a>
                    ) : (
                      <Link href={item.link || '#'} target={target} rel={rel}>{item.text}</Link>
                    )}
                  </li>
                );
              })}
